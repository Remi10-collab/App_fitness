  ...
  return `<div class="card"><h2>Ajouter une série</h2>
    <div class="train-top-row">
      ...
      <div class="train-field train-field-type">
        <label>Type</label><select id="trType"><option selected>1</option><option>2</option><option>3</option><option>4</option></select>
      </div>
    </div>
	...
	<div class="autocomplete autocomplete-clearable">
	  <input
		...
		oninput="filterExercises();updateAutocompleteClearButton('trExercice','clearExerciseBtn')"
		autocomplete="off"
	  >
      <button type="button" id="clearExerciseBtn" class="autocomplete-clear-btn" hidden aria-label="Effacer l’exercice sélectionné" title="Effacer l’exercice" onclick="clearExerciseSelection()">×</button>
  ...
  return `<div class="card"><h2>Ajouter un aliment</h2>
    ...
    <div class="autocomplete autocomplete-clearable">
      <input
        id="mealFood"
        placeholder="Tape au moins 2 lettres..."
        oninput="filterFoods();updateAutocompleteClearButton('mealFood','clearFoodBtn')"
        autocomplete="off"
      >
      <button type="button" id="clearFoodBtn" class="autocomplete-clear-btn" hidden aria-label="Effacer l’aliment sélectionné" title="Effacer l’aliment" onclick="clearFoodSelection()">×</button>
...
function profileHtml(){
  ...
  const pseudo = (profile.pseudo || "").trim() || "Profil utilisateur";
  ...
  return `<div class="card profile-personal-card ${profileFormOpen ? "is-open" : ""}">
    <div class="profile-personal-head" role="button" tabindex="0" onclick="toggleProfileForm()" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleProfileForm();}" aria-expanded="${profileFormOpen ? "true" : "false"}">
      <div>
        <h2>Informations personnelles</h2>
        <strong>${escapeHtml(pseudo)}</strong>
      </div>
      <span class="collapse-arrow">${profileFormOpen ? "▲" : "▼"}</span>
    </div>
    ${profileFormOpen ? profileForm : `<div class="profile-summary profile-summary-page profile-collapsed-summary">
...
function updateAutocompleteClearButton(inputId, buttonId){
